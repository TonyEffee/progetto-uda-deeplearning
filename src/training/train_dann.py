...
Training del modello con DANN (Domain-Adversarial Neural Network) come
seconda tecnica di Unsupervised Domain Adaptation (Obiettivo Minimo 3).
...
La loss totale e':
    ...
    loss = cross_entropy(source_logits, source_labels)
           + domain_cross_entropy(source_domain_logits, "source")
           + domain_cross_entropy(target_domain_logits, "target")

con il coefficiente della Gradient Reversal Layer (lambda) che cresce
gradualmente da 0 a 1 nel corso del training (schedula standard proposta
nel paper originale), per stabilizzare le prime iterazioni quando le
feature non sono ancora affidabili.
...
def get_lambda_schedule(progress, gamma=10.0):
    """
    Schedula standard del paper DANN originale: lambda cresce da 0 a 1 in
...
def train_one_epoch(model, domain_discriminator, source_loader, target_iterator,
    ...
    for batch_idx, (source_images, source_labels) in enumerate(progress_bar):
        ...
        lambda_grl = get_lambda_schedule(progress)
